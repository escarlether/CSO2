...
int main() {
    //I calculated the number of entries in the bigger cache by doing 
    // 32KB / 64B = (2^15)/(2^6) = 2^9 = 512
    // 512 * 16 should be the max index because there are 16 indices in every entry and we need 512 entries
    const int MAX = 16 * 512;
    //This is the size of every block in entries
    const int SKIP = 16;
    //this is more than 10 million so it meets the quota
